...
distinct_colors = [ImageColor.getrgb(i) for i in distinct_hex]
...
def test_input_dataloader(dataloader):
...
def test_input_dataset(dataset):
    ...
    # height, width = y["size"]["height"], y["size"]["width"]
    ...
    for obj in y:
        ...
        label = rev_label_map[int(obj[0])]

        img = cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0,255,255), thickness=2)
        img = cv2.putText(img, label, (xmin, ymin-10), font, fontScale, (0,255,255))
...
def transforms(img, ann):
    ...
    for obj in objects:
        ...
        bndbox = obj["bndbox"]
        xmin = int(bndbox["xmin"])/width
        ymin = int(bndbox["ymin"])/height
        xmax = int(bndbox["xmax"])/width
        ymax = int(bndbox["ymax"])/height
    ...
    transformed = transform_albu(image=img, bboxes=labels)
    ...
    labels = torch.tensor(labels, dtype=torch.float32).roll(1, dims=1)

    return toTensor(img), labels

#@save
def multibox_prior(data, sizes, ratios):
    """Generate anchor boxes with different shapes centered on each pixel."""
    in_height, in_width = data.shape[-2:]
    device, num_sizes, num_ratios = data.device, sizes.shape[0], ratios.shape[0]
